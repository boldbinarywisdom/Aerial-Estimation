...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //Using current attitude estimates to map to phi and theta
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  // We derive Rbg prime which is matrix for body frame to global frame transformed
  // Reference section 7.2 from Estimation for Quadcopters paper
  // Store pitch, roll and yaw in interim variables for using in matrix later
  //
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  // For predict we need state transition function
  ...
  ekfCov = gPrime * ekfCov * gPrime.transpose() + Q; //model coveriance member var
...
void QuadEstimatorEKF::UpdateFromGPS(V3F pos, V3F vel)
{
  ...
  // section 7.3.1 from Estimation for Quadcopters
